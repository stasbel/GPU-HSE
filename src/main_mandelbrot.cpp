...
int main(int argc, char **argv) {
...

    {
        ...
        std::cout << "CPU: " << (float) maxApproximateFlops / gflops / t.lapAvg() << " GFlops" << std::endl;
    ...
    context.activate();
    {
        ...
        gpu::gpu_mem_32f kernel_results;
        kernel_results.resizeN(width * height);
        ...
        kernel_results.readN(gpu_results.ptr(), width * height);

        size_t flopsInLoop = 10;
        size_t maxApproximateFlops = width * height * iterationsLimit * flopsInLoop;
        size_t gflops = 1000 * 1000 * 1000;
        std::cout << "GPU: " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
        std::cout << "GPU: " << (float) maxApproximateFlops / gflops / t.lapAvg() << " GFlops" << std::endl;
...
void renderInWindow(float centralX, float centralY, unsigned int iterationsLimit, bool useGPU) {
    ...
    do {
        ...
        if (window.isResized()) {
            window.resize();
            width = (unsigned int) window.width();
            height = (unsigned int) window.height();
